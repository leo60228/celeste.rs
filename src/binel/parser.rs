...
pub fn take_string(input: &[u8]) -> IResult<&[u8], String> {
    do_parse!(
        input,
        length: take_varint >> string: take_str!(length) >> (string.to_string())
    )
}
...
pub fn take_rle_char(buf: &[u8]) -> IResult<&[u8], String> {
    do_parse!(
        buf,
        times: le_u8 >> byte: le_u8 >> ((byte as char).to_string().repeat(times as usize))
    )
}
...
pub fn take_rle_string(buf: &[u8]) -> IResult<&[u8], String> {
    do_parse!(
        buf,
        len: le_i16 >> chars: count!(take_rle_char, (len / 2) as usize) >> (chars.concat())
    )
}
    ...
    do_parse!(
        ...
        val: alt!(
            do_parse!(tag!(b"\x00") >> byte: le_u8 >> (BinElAttr::Bool(byte != 0)))
                ...
                | do_parse!(
                    tag!(b"\x05")
                        >> string: apply!(take_lookup, lookup)
                        >> (BinElAttr::Text(string.to_string()))
                )
                | do_parse!(
                    tag!(b"\x06") >> string: take_string >> (BinElAttr::Text(string.to_string()))
                )
                | do_parse!(
                    tag!(b"\x07")
                        >> string: take_rle_string
                        >> (BinElAttr::Text(string.to_string()))
                )
        ) >> ({
            debug!("{:?}", val);
